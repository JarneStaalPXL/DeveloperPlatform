<template>
  <n-config-provider>
    <n-grid x-gap="24" y-gap="24" cols="1 680:2 1200:4">
      <n-gi v-for="font of $store.state.fonts" :key="font">
        <n-card :title="font.name" bordered>
          <template #cover>
            <div
              class="cover"
              :style="{
          ...
          <section class="toolContent">
            <div class="titleButtons">
              <div class="actionBtns">
                <n-button @click="openLink(font.link)">Open website</n-button>
                <n-button
                  class="pl-5"
                  @click="addToolToFavorites(font)"
                  v-if="!font.isFavorited"
                  ><i class="fa-solid fa-heart"></i
                ></n-button>
                <n-button
                  class="pl-5"
                  @click="removeToolFromFavorites(font)"
                  v-if="font.isFavorited"
            ...
            <div class="justify-content-end promoContainer" v-if="font.promoDescription">
              <n-button class="promoBtn" @click="openLink(font.promoLink)">{{
                font.promoDescription
              }}</n-button>
            </div>
          </section>
        </n-card>

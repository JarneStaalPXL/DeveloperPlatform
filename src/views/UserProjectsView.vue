  ...
  <n-card class="categoryCard">
    <template #header>
      <div class="d-flex" style="gap: 10px">
        <h4>Projects</h4>
        <n-badge
          v-if="$store.state.userProjects"
          :value="$store.state.userProjects.length"
          color="grey"
        />
      </div>
    </template>
    ...
    <UserProjectsFull
      v-if="
        $store.state.userProjects !== undefined && $store.state.userProjects.length > 0
      "
    />
    <div v-else>
...
<script>
import { NCard, NBadge, NButton } from "naive-ui";
